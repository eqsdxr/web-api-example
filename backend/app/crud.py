...
from app.models import User, UserCreate, UsersPublic, UserResponse
...
def retrieve_users(session: Session, offset: int = 0, limit: int = 100) -> UsersPublic:
    db_users = session.exec(select(User).offset(offset).limit(limit))
    public_users = [UserResponse(**db_user.model_dump()) for db_user in db_users]
